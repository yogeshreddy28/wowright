...
import { shopFilterHref } from '@/lib/catalog-links';
...
export function ShopFilters({
  ...
  function update(name: string, value: string, debounce = false) {
    const commit = () => {
      router.push(
        shopFilterHref(pathname, searchParams.toString(), name, value),
        { scroll: false },
      );
      trackCommerce('shop_filter_used', { filter: name, value });
    };
    if (timer.current) clearTimeout(timer.current);
  ...
  return (
    ...
    >
      <div className="shop-mobile-controls" aria-label="Shop controls">
        <label className="shop-mobile-search">
        ...
        >
          <SlidersHorizontal /> Filter & category
        </button>
        <label>
      ...
      <div
        className={`shop-filter-fields${mobileOpen ? ' open' : ''}`}
      ...
      >
        ...
        <details
          className="ux-shop-advanced"
          open={Boolean(values.finish || values.min || values.max)}
        >
        ...
        <p className="catalog-filter-status" aria-live="polite">
