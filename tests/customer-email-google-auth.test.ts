...
describe('customer email authentication', () => {
  it('securely claims an assisted-order customer only after matching email verification', async () => {
    ...
    const password = 'secure-claim-password';
    ...
    expect(
      (
        await login(
          post('/api/account/login', { email: 'guest@example.com', password }),
        )
      ).status,
    ).toBe(403);
    const customer = database.sqlite
      .prepare(
        "SELECT id,account_claim_pending FROM customers WHERE email_normalized='guest@example.com'",
      )
      .get() as { id: string; account_claim_pending: number };
    expect(customer).toEqual({ id: 'assisted', account_claim_pending: 1 });
  ...
  it('sends branded transactional email only through server-side provider configuration', async () => {
    ...
    const fetcher = vi.fn(
      async (_url: string | URL | Request, init?: RequestInit) => {
        ...
        expect(body).toMatchObject({
          from: 'WOW RIGHT <account@example.com>',
    ...
    await expect(
      sendVerificationEmail('person@example.com', 'verification-token'),
  ...
  it('requires a valid email and permits login before and after verification', async () => {
    ...
    const before = await login(
      post('/api/account/login', { email: 'person@example.com', password }),
    );
    expect(before.status).toBe(200);
    expect(
      ((await before.json()) as { emailVerified: boolean }).emailVerified,
    ).toBe(false);
    const customer = database.sqlite
      .prepare('SELECT id,email_normalized FROM customers')
      .get() as { id: string; email_normalized: string };
    ...
    expect(
      (await verifyEmail(post('/api/account/verify-email', { token }))).status,
    ).toBe(200);
    expect(
      (
        (await (
  ...
  it('uses expiring single-use reset tokens and invalidates existing sessions', async () => {
    ...
    const customer = database.sqlite
      .prepare(
        "SELECT id FROM customers WHERE email_normalized='reset@example.com'",
      )
      .get() as { id: string };
    const expired = await createCustomerAuthToken(
      database.db,
      customer.id,
      'reset_password',
      60_000,
    ...
    database.sqlite
      .prepare(
        "UPDATE customer_auth_tokens SET expires_at='2000-01-01' WHERE purpose='reset_password'",
      )
      .run();
    expect(
      (
    ...
    const valid = await createCustomerAuthToken(
      database.db,
      customer.id,
      'reset_password',
      60_000,
    ...
    expect(
      (
        await login(
          post('/api/account/login', { email: 'reset@example.com', password }),
        )
      ).status,
    ).toBe(401);
    expect(
  ...
  it('does not reveal whether a forgot-password email exists', async () => {
    ...
    const known = await forgotPassword(
      post('/api/account/forgot-password', { email: 'known@example.com' }),
    );
    const unknown = await forgotPassword(
      post('/api/account/forgot-password', { email: 'unknown@example.com' }),
    );
...
describe('Google OIDC account flow', () => {
      ...
      vi.fn(async (url: string) => {
        if (url.includes('/tokeninfo'))
          return Response.json({
            ...
            email_verified: 'true',
            name,
            nonce: database.sqlite
              .prepare(
                'SELECT nonce FROM google_oauth_states ORDER BY created_at DESC LIMIT 1',
              )
              .get()!.nonce,
          });
  ...
  it('creates a password-free Google customer and stores the verified email', async () => {
    ...
    const callback = await googleCallback(
      new Request(
        `http://local/api/account/google/callback?state=${auth.state}&code=code`,
        { headers: { cookie: `wow_google_oauth=${auth.state}` } },
      ),
    );
    expect(callback.status).toBe(302);
    const completed = await googleComplete(
      post(
        '/api/account/google/complete',
        { name: 'Google Person', phone: '9000000034' },
        `wow_google_oauth=${auth.state}`,
      ),
    );
    expect(completed.status).toBe(200);
    const row = database.sqlite
      .prepare(
        "SELECT email,email_verified_at,auth_method,google_subject,password_hash FROM customers WHERE email_normalized='google@example.com'",
      )
  ...
  it('logs a returning Google customer in without creating a duplicate', async () => {
    database.sqlite.exec(
      "INSERT INTO customers(id,name,mobile,email,email_normalized,email_verified_at,auth_method) VALUES('returning','Returning','919000000035','returning@example.com','returning@example.com','2026-01-01','email')",
    );
    ...
    expect(
      database.sqlite.prepare('SELECT count(*) n FROM customers').get()!.n,
    ).toBe(1);
    expect(
      database.sqlite
        .prepare("SELECT google_subject FROM customers WHERE id='returning'")
        .get()!.google_subject,
    ).toBe('returning-google');
  });
});
